...
from securicad.azure_collector.schema_classes import Disk
...
def parse_obj(resource, resource_type, resource_group, sub_id, name, rg_client, rg_query_options, resource_id) -> Disk:
    ...
    try:
        ...
        log.warning(
            f"Couldn't execute resource graph query of disk {name}. Possibly a vhd stored in storage account"
        )
        os = None
    kind = (
        "OsDisk"
