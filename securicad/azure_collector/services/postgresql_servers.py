...
from securicad.azure_collector.schema_classes import PostgreSQLDatabase
from . import network_handling
from securicad.azure_collector.services.parser_logger import log
import azure.mgmt.resourcegraph as arg
import requests

def parse_obj(resource_type, resource_group, sub_id, name, rg_client, rg_query_options, resource_id, DEBUGGING, headers) -> PostgreSQLDatabase:
    ...
    try:
        ...
        log.error(
            f"Couldn't execute resource graph query of db for postgreSQL {name}, skipping asset."
        )
        return None
    ...
    try:
        ...
        log.error(
            f"Error requesting GET {endpoint}. Could be a bad authentication due to Bearer token."
        )
    firewallRules = []
    ...
    if raw_firewallRules_data:
        for raw_firewallRule in raw_firewallRules_data:
            try:
                ...
                log.debug(
                    f"Could not get start ip from firewall rule {raw_firewallRule} in postgresql-server {name}."
                )
            try:
                ...
                log.debug(
                    f"Could not get end ip from firewall rule {raw_firewallRule} in postgresql-server {name}."
                )
