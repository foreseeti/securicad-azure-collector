        ...
        try:
            ...
            for entry in data.get("value", []):
                ...
                try:
                    ...
                    if debugging:
                        print(f"WARNING: Couldn't see member type of member {id} in {group_id}. Impact: Possibly missing association between security principal and group." )
                members.append({"id": id, "memberType": member_type})
        except:
            if debugging:
                print(f"Couldn't json-encode the request response in {__name__}")
    else:
        if debugging:
            print(f"WARNING: Status code {res.status_code} on {url}. Impact: Couldn't get members of {group_id}, ensure correct API permissions for the collector on Microsoft Graph.")
    return members